...
do
  ...
  oc image mirror -a ~/mirror_pullsecret_config.json $line $MIRROR_REGISTRY/$namespace/$imagename
  if [ $? -ne 0 ]; then
...
do
  oc image mirror -a ~/mirror_pullsecret_config.json quay.io/openshift-release-dev/$item ${MIRROR_REGISTRY}/openshift-release-dev/ocp-v4.0-art-dev
  if [ $? -ne 0 ]; then
    echo -e "Image mirrors failed, please try it again.\n"
  fi
done
